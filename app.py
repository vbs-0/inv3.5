...
def add_bus():
    ...
    if request.method == 'POST':
        ...
        bus = Bus(name=name, description=description)
        db.session.add(bus)
        db.session.commit()
...
def assign_parts():
    ...
    if request.method == 'POST':
        ...
        bus_part = BusPart(bus_id=bus_id, product_id=product_id, quantity=quantity)  # Use product_id
...
def assigned_to():
    ...
    assignments = db.session.query(
        ...
        BusPart.quantity,
        User.name.label('assigned_by'),
        BusPart.time.label('date_assigned')  # Include the time column
    ).join(Bus).join(Product).join(UserActivity, UserActivity.user_id == current_user.id).distinct().all() # Explicit join
